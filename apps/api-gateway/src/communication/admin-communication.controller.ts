...
export class AdminCommunicationController {
  ...
  async sendAdminNotification(@Req() req: Request, @Res() res: Response) {
    try {
      const response = await axios.post(`${this.internalUrl}/notifications/admin/send`, req.body);
      return res.status(response.status).json(response.data);
    } catch (error: any) {
      const status = error.response?.status || HttpStatus.INTERNAL_SERVER_ERROR;
      const data = error.response?.data || { message: 'Failed to send admin notification' };
